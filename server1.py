...
def wlantemplate():
    if "user" in session:
        user = session.get('user')
        return render_template("wlantemplate.html", user=user)
...
def nopassword():
    if "user" in session:
        user = session.get('user')
        return render_template("nopassword.html", user=user)
    else:
        return redirect(url_for("index"))  # Redirect to login if not logged in
...
def psk():
    if "user" in session:
        user = session.get('user')
        return render_template("psk.html", user=user)
    else:
        return redirect(url_for("index"))  # Redirect to login if not logged in
...
def login():
    ...
    if user and check_password_hash(user['password'], password):
        session['user'] = {"id": str(user['_id']), "username": user['username'], "role": user['role'], "userKey":user['userKey'] }
...
def history():
    ...
    return render_template("/history.html", documents=documents, users=users)
...
def add_users(GroupandOU):
    ...
    try:
        ...
        for user in users:
            ...
            # Connect to LDAP server
            s = Server(server_address, connect_timeout=5, use_ssl=True, get_info=ALL)
            c = Connection(s,  user=f"{domain}\\{loginun}",password=loginpw,auto_bind=True,)
...
# get user ----------------------------------------------------------------------------------------------------
@app.route("/alllocaluser", methods=["GET"])
def get_localuser_mongo():
    try:
        # Retrieve all documents with status 'Active' from the collection
        query = {"userKey": "1"}
        documents = db.users.find(query)
...
def get_detail_mongo(_id):
    try:
        ...
        return jsonify({"error": str(e)}), 500
# get getusers_mongo in db ----------------------------------------------------------------------------------------------------
# get localuser detail in db ----------------------------------------------------------------------------------------------------
@app.route("/getdetail_localuser/<_id>", methods=["GET"])
def getdetail_localuser(_id):
    try:
        # Convert string _id to ObjectId
        document = db.users.find_one({"_id": ObjectId(_id)})
...
def update_user(user_id):
    ...
    if 'userKey' in data:
        update_data['userKey'] = data['userKey']
